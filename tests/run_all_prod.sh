...
##############################################################################
# FormBridge End-to-End Test Suite - PRODUCTION
# Runs comprehensive tests against production API
# Usage: bash tests/run_all_prod.sh
##############################################################################
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  echo "   Please copy tests/.env.prod.example to tests/.env.prod and fill in values"
  exit 1
fi
...
print_banner() {
  echo -e "${CYAN}"
  echo "╔══════════════════════════════════════════════════════════════╗"
  echo "║  FormBridge End-to-End Test Suite (PRODUCTION)              ║"
...
sanity_checks() {
  ...
  if [[ -z "$API_KEY" ]] || [[ "$API_KEY" == "REPLACE_ME" ]]; then
    echo -e "${RED}✗ API_KEY not configured${NC}"
    record_step "sanity_checks" "FAIL" 0 "API_KEY not set"
    return 1
  fi
  ...
  for tool in node jq curl aws; do
  ...
  echo "  API_KEY: $(echo $API_KEY | head -c 4)...${API_KEY: -4}"
  ...
  if aws sts get-caller-identity --region "$REGION" > /dev/null 2>&1; then
    local identity=$(aws sts get-caller-identity --region "$REGION" --output json | jq -r '.Arn')
    echo -e "${GREEN}✓${NC} AWS authenticated: $identity"
  else
    echo -e "${RED}✗ AWS authentication failed${NC}"
    record_step "sanity_checks" "FAIL" 0 "AWS credentials invalid"
    return 1
  fi
  ...
  if curl -s --connect-timeout 5 -H "X-Api-Key: $API_KEY" "$BASE_URL" > /dev/null 2>&1 || \
...
##############################################################################
# Test Steps (same as local but with prod-specific variations)
##############################################################################
  ...
  local payload=$(cat <<EOF
{
  ...
  "message": "Production test submission from test suite",
  "timestamp": $(date +%s)
}
EOF
)
...
test_hmac() {
  ...
  if [[ -z "$HMAC_SECRET" ]] || [[ "$HMAC_SECRET" == "REPLACE_IF_ENABLED" ]]; then
    echo -e "${YELLOW}⚠ HMAC enabled but secret not configured${NC}"
    record_step "hmac" "PASS" 0 "Skipped (secret not set)"
    return 0
  fi
  ...
  local payload=$(cat <<EOF
{
  ...
  "message": "Testing HMAC signature in production",
  "timestamp": $(date +%s)
}
EOF
)
...
test_ses() {
  print_step "TEST: SES Email Status"

  local start_time=$(date +%s%N)

  echo "Checking SES send statistics in region: $REGION"

  # Get SES send stats
  local stats=$(ses_get_send_stats "$REGION" 2>/dev/null)

  if echo "$stats" | jq -e '.SendDataPoints[0]' > /dev/null 2>&1; then
    local bounces=$(echo "$stats" | jq '[.SendDataPoints[].Bounces] | add')
    local complaints=$(echo "$stats" | jq '[.SendDataPoints[].Complaints] | add')
    echo -e "${GREEN}✓ SES stats retrieved${NC}"
    echo "  Recent bounces: $bounces"
    echo "  Recent complaints: $complaints"
    
    local duration_ms=$(( ($(date +%s%N) - start_time) / 1000000 ))
    record_step "ses_status" "PASS" $duration_ms "{\"bounces\":$bounces,\"complaints\":$complaints}"
    return 0
  else
    echo -e "${YELLOW}⚠ No SES data found${NC}"
    
    local duration_ms=$(( ($(date +%s%N) - start_time) / 1000000 ))
    record_step "ses_status" "PASS" 0 "No recent activity"
    return 0
  fi
}
...
main() {
  ...
  # Initialize summary
  node "$SCRIPT_DIR/lib/init_summary.js" "$SUMMARY_FILE" "prod" "$BASE_URL"
